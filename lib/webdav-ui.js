...
function handleFileAction(action, id, event, type) {
	...
	if (action == '--') {   
		...
		if (action == 'rename') {
			...
			if (window.confirm(cfmsg)) document.getElementById('faform').submit();
		}
	} else if (type == 'rename') {
		if (event.keyCode == 13) {
			if (event.preventDefault) event.preventDefault(); else event.returnValue = false;
...
function setTextAreaCookie(eid) {
	...
	var ff = e.className.indexOf(' mono')>-1 ? 'mono' : 'sans';
...
function handleTextAreaFontFamily(eid,tid) {
	...
	toggleClassName(e, 'sans', ismono);
	toggleClassName(e, 'mono', !ismono);
	toggleClassName(t, 'sans', ismono);
