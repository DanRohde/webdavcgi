...
sub getSmbConfig {
    ...
    my $smb;
    if ( $$self{memcached} ) {
        my $cache = new Cache::Memcached { servers => [ $$self{memcached} ],
            debug => $$self{debug} };
...
sub _getSmbConfig {
    ...
    $self->debug(
        "genConfig: homeDirectory: $homeDirectory, homeDrive: $homeDrive, other=$otherDirectory"
    );
    ...
    {                              # wenn es ein korrekter Shareeintrag ist
        ...
        $SMB{domains}->{$domain}->{fileserver}->{$homeServer}->{shares}
            = [$homeShare];
        $SMB{domains}->{$domain}->{fileserver}->{$homeServer}->{sharealiases}
            ->{$homeShare} = "$homeDrive \u$homeShare/";
        $SMB{domains}->{$domain}->{fileserver}->{$homeServer}->{initdir}
            ->{$homeShare} = $homeDir
            if $homeDir;
    }
    ...
    foreach ( split( /\r?\n/, $otherDirectory ) ) {
        ...
        if (/^($regex_drive)?\\\\($regex_domain)\\([^\\]+)(\\.*)?$/) {
            ...
            $alias =~ s/.*\///;
            push(
                @{  $SMB{domains}->{$domain}->{fileserver}->{$server}
                        ->{shares}
                },
                $share . $directory
            );
