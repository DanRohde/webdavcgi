$(document).ready(function(){function b(e,f){$(".error",f).removeClass("error");d(f);var g=e.split(",");g.forEach(function(h){$("input[name="+h+"],textarea[name="+h+"]",f).addClass("error")});$("input[name="+g[0]+"]",f).focus()}function d(e){$("#inputfailure",e).show(1000);window.setTimeout(function(){$("#inputfailure",e).hide(800)},5000)}function c(f,e){if($("input[name="+f+"],textarea[name="+f+"]",e).val().trim()==""){b(f,e);return false}return true}function a(e,g){var f;for(f=0;f<e.length;f++){if(!c(e[f],g)){return false}}return true}$("body").on("fileActionEvent",function(e,f){if(f.obj.hasClass("disabled")){return}if(!f.obj.hasClass("sendbymail")){return}var h=window.location.pathname;var g=ToolBox.blockPage();$(".action.sendbymail").addClass("disabled");$.post(h,{action:"sendbymail",ajax:"preparemail",files:f.selected},function(i){var j=$(i);g.remove();$(".action.sendbymail").removeClass("disabled");$("input[name=download]",j).click(function(k){k.preventDefault();j.dialog("close");$("form",j).off("submit").append('<input type="hidden" name="download" value="yes"/>').submit();return false});$("input[name=to],textarea[name=to],textarea[name=cc],textarea[name=bcc]",j).autosize().autocomplete({minLength:4,autoFocus:true,source:function(n,l){var k=n.term.split(/\s*,\s*/);var m=k.pop().trim();if(m!=""&&m.length>3){$.post(window.location.pathname,{action:"sendbymail",ajax:"addressbooksearch",query:m},function(o){l(o.result?o.result:new Array())})}else{l(new Array())}},select:function(n,m){var l=$(this);ToolBox.preventDefault(n);var k=l.val().split(/\s*,\s*/);k.pop();k.push(m.item.value);l.val(k.join(",\r\n")+"\r\n");return false},focus:function(l,k){ToolBox.preventDefault(l);return false}});$("input",j).on("keydown",function(k){$(this).next("input").focus();return k.which!==13});$("input[name=zipfilename]",j).prop("disabled",true);$("input[name=zip]",j).change(function(){if($(".sendbymail.files.dir",j).length>0){$(this).prop("checked",true)}$("input[name=zipfilename]",j).prop("disabled",!$(this).is(":checked"))});if($(".sendbymail.files.dir",j).length>0){$("input[name=zip]",j).prop("checked",true).trigger("change")}if($(".sendbymail.remove",j).length==1){$(".sendbymail.remove",j).remove()}$(".sendbymail.remove",j).click(function(){$(".sendbymail.sumfilesizes").remove();$(this).parent().remove();if($(".sendbymail.remove",j).length==1){$(".sendbymail.remove",j).remove()}});$("form.sendbymail",j).submit(function(){$("input.error",j).removeClass("error");if(!a(new Array("from","to","subject"),j)){return false}$("input[type=submit]",j).prop("disabled",true);var l=ToolBox.blockPage();var k=$.post(h,$(this).serialize(),function(o){l.remove();var m="info",n=o.msg;if(o.error){m="error";n=o.error;if(o.field){b(o.field,j)}}noty({text:n,type:m,layout:"topCenter",timeout:30000});if(!o.error){j.dialog("close")}else{$("input[type=submit]",j).prop("disabled",false)}});ToolBox.renderAbortDialog(k);return false});$("input[type=submit]",j).button();$(".sendbymail.addcc,.sendbymail.addbcc",j).button().on("click",function(){var k=$(this);if(k.hasClass("addcc")){$("form.sendbymail").addClass("showcc").focus()}if(k.hasClass("addbcc")){$("form.sendbymail").addClass("showbcc").focus()}});j.dialog({modal:true,width:"auto",height:"auto",dialogClass:"sendbymail",closeText:$("#close").html()});j.MyTooltip(500)})})});
/*!
