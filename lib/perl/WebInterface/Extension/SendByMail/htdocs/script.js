...
$(document).ready(function() {
	...
	$("body").on("fileActionEvent", function(event,data) {
		...
		$.post(loc, { action: 'sendbymail', ajax: 'preparemail', files: data.selected}, function(response) {
			...
			$("input[name=to],textarea[name=to]", dialog).autosize().autocomplete(
						{ minLength: 4, 
							source: function(request,response) {
								$.post(window.location.pathname, {action:'sendbymail', ajax: 'addressbooksearch', query: request.term}, function(resp) {
