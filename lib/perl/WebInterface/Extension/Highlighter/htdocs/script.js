...
$(document).ready(function() {
	...
	$("body").on("fileActionEvent", function(event,data) {
		...
		if (data.obj.hasClass("mark")) {
		...
		} else if (data.obj.hasClass("replacemarks")) {
			var files = ToolBox.getSelectedFiles(data.obj);
			var rows = ToolBox.getSelectedRows(data.obj);
			...
			var rows = ToolBox.getSelectedRows(data.obj);
			var styles = $('.highlighter-popup .removemarks').data("styles").split(",");
			for (var i = 0; i<styles.length; i++) {
				$("td,a.changeuri",rows).css(styles[i],"");
				removeHighlighterData(rows, styles[i]);
			}
			...
			$.post(window.location.pathname, { action: "replacemarks", style: JSON.stringify(self.data("style")), files: ToolBox.getSelectedFiles(data.obj)}, myResponseHandler);
	...
	function handlePresetChanges() {
		$.get(window.location.pathname, {}, function(resp) {
			$(".highlighter.preset").html($(".highlighter.preset",resp).html());
			ToolBox.initPopupMenu();
		});
	}
