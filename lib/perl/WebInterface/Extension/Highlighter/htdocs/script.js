	...
	$("body").on("fileActionEvent", function(event,data) {
		...
		if (data.obj.hasClass("mark")) {
			...
			for (var i = 0; i < sortedPresetKeys.length; i++) { 
				...
				var entry = $(entrytmpl);
				$(".highlighter-preset-entry-name", entry).text(preset).attr("title",styles).css(JSON.parse(styles));
				$(".highlighter-preset-entry-delete", entry).attr("data-preset",preset).on("click", function(ev) {
					...
					if (window.confirm(self.data("confirm").replace('%s',preset))) {
			...
			dialog.dialog({modal:true, width: 'auto', dialogClass: "highlighter-preset-manager-dialog", close: function() { window.location.reload(true); }});
		}
	});
