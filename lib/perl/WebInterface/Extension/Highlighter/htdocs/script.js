...
$(document).ready(function() {
	...
	$(".action.mark").on("click", function(event) {
		...
		$("td,a.changeuri", rows).css(style, value);
	...
	$(".action.replacemarks").on("click", function(event) {
		...
		if (data) {
			...
			$.post(window.location.pathname, { action: 'replacemarks', files: files, style: JSON.stringify(data)}, myResponseHandler);
	...
	function initPresetActions() {
		$(".action.markwithpreset").on("click", function(event) {
			var self = $(this);
			var rows = ToolBox.getSelectedRows(this);
			...
			$.post(window.location.pathname, { action: "replacemarks", style: JSON.stringify(self.data("style")), files: ToolBox.getSelectedFiles(this)}, myResponseHandler);
		});
		$(".action.managepresets").on("click", function(event) {
			var self = $(this);
	...
	function handlePresetChanges() {
		$.get(window.location.pathname, {}, function(resp) {
			$("#popupmenu .highlighter.preset").html($("#popupmenu .highlighter.preset",resp).html());
			$(".toolbar .highlighter.preset").html($(".toolbar .highlighter.preset",resp).html());
			initPresetActions();
		});
	}
