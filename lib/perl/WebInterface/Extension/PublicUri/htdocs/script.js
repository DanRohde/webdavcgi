...
$(document).ready(
		function() {
			$("extension").on("click", handlePublicUriFileActionEvent);
			...
			function handleFilePuri(row) {
				row.fadeTo('slow', 0.5);
				confirmDialog($('#purifileconfirm').html().replace(/%s/,
						simpleEscape(row.attr('data-file'))), {
					confirm : function() {
						var file = row.attr('data-file');
						updateFileList(); // FIXME: refresh only the file!
						$.post($('#fileList').attr('data-uri'), {
							'puri' : 'yes',
							file : file
						}, function(response) {
							if (response.error)
			...
			function handleFileDepuri(row) {
				row.fadeTo('slow', 0.5);
				confirmDialog($('#depurifileconfirm').html().replace(/%s/,
						simpleEscape(row.attr('data-file'))), {
					confirm : function() {
						var file = row.attr('data-file');
						updateFileList(); // FIXME: refresh only the file!
