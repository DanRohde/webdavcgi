...
$(document).ready(function() {
	...
	function doSaveTextData(row,text) {
		...
		var xhr = $.post(ToolBox.addMissingSlash($('#fileList').attr('data-uri')), { action: 'savetextdata', filename: row.attr('data-file'), textdata: text.val() }, function(response) {
			if (!response.error && response.message) {
				text.data("response", text.val());
				//updateFileList();
				var xhr = $.get(ToolBox.addMissingSlash($('#fileList').attr('data-uri')), { ajax: 'getFileListEntry', template: $('#fileList').attr("data-entrytemplate"), file: row.attr('data-file')}, function(r) {
					var newrow = $(r);
