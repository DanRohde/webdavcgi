...
$(document).ready(function() {
	...
	function doSaveTextData(datauri, filename, text, size) {
		...
		var xhr = $.post(ToolBox.addMissingSlash(datauri), { action: 'savetextdata', filename: filename, textdata: text.val() }, function(response) {
	...
	function handleFileEdit(row) {
		...
		var xhr = $.get(datauri, { action: 'edit', filename: filename }, function(response) {
			block.remove();
			if (response.message || response.error) {
				...
				dialog.find('.action.savetextdata').button().off('click').click(function(event) {
					...
					if (ToolBox.cookie("settings.confirm.save") != "no") 
						ToolBox.confirmDialog($('#confirmsavetextdata').html().replace(/%s/,row.attr('data-file')), { 
