...
(function($) {
    ...
    $.fn.renderMarkdown = function(val) {
    	...
    	var blocks = [];
		val = val
                .replace(/^\s{4}\s*([^*\-].*)$/mg, function(t) { blocks.push("<div class='markdown-codeblock'>"+encodeHTML(t.substr(4))+"</div>");  return 'BlX('+blocks.length+')BlX';}) // code block: 4 spaces ...
        ...
        for (var i=0, length = lines.length; i< length; i++) {
            ...
            if (m = line.match(/^([*\-]|\d+[.]) /)) { // list item found
                var listtype = m[0].match(/\d+/) ? 'ol' : 'ul';
                if (listtypes.length == 0 || listtypes[0] != listtype) { // start new list (no list yet or a type change)
                    if (listtypes.length>0) { // looks like a type change
                    ...
                    listlevels.push(0);
                    text += '<'+listtypes.last()+' class="markdown-'+listtypes.last()+'">';
                    line = line.replace(/^([*\-]|\d+[.]) (.*)$/mg, "<li>$2");
                } else {
                    while (listtypes.length > 1) { // close sub lists
                        text+='</li></'+listtypes.pop()+'>';
                        listlevels.pop();
                    }
                	line = line.replace(/^([*\-]|\d+[.]) (.*)$/mg, "</li><li>$2");
                }
            } else {
                if (!line.match(/^\s+/)) { // list ends:
