    ...
    $.fn.renderMarkdown = function(val) {
        ...
        function renderTable(txt) {
            ...
            for (var i=0,l=lines.length; i<l; i++) {
                ...
                var row = '';
                var celltype = 'td';
                if ( line.match(/^[\s\-:|]+$/) || line.match(/^\s*$/) ) { // skip column defs and empty lines
                    continue;
                } else if (i+1<l && lines[i+1].match(/^[\s\-:|]+$/)) { // look forward for column defs
                    celltype = 'th';
                ...
                if (cells.length>0) {
                    ...
                    $.each(cells, function(ci,cell) { 
                        row+='<'+celltype+' class="markdown-td'+(cellclasses[ci] ? ' '+cellclasses[ci] : '')+(celltype=='th'? ' ui-widget-header':' ui-widget-content')+'">'
                            +(cell.match(/^\s*$/) ? '' : $("<div/>").renderMarkdown(cell).html())
                            +'</'+celltype+'>';
                    });
                    if (celltype=='th') table+='<thead><tr class="markdown-thead">'+row+'</tr></thead>';  else table+='<tr class="markdown-tr">'+row+'</tr>';
                }
            }
        ...
        val = val
                ...
                .replace(/^\s*[\-*] [\-*] [\-*]( [\-*])*\s*$/mg, "<hr>")                // horizontal rules: - - - or * * *
        ...
        for (var i=0, length = lines.length; i< length; i++) {
            ...
            if ( (m = line.match(listregex)) ) { // a (sub) list
