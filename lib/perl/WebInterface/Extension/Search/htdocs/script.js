...
$(document).ready(function() {
	...
	function initResult(result) {
		...
		.attr("data-fancybox-group","searchresultgallery")
		.fancybox({
			padding: 0,
			afterShow: function() { $(".fancybox-close").focus();},
			beforeLoad: function() { 
				this.title = $(this.element).siblings(".label").html();
				this.content = $(this.element).clone().removeAttr("class").attr("src", $(this.element).attr("src").replace(/\?action=thumb$/,""));
			}, 
			helpers:{ buttons: {}, 
						thumbs: { 
							width: 60, 
							height: 60, 
							source: function(current) { return $(current.element).attr("src")+'?action=thumb'; } 
						}
			} 
		});
	}
