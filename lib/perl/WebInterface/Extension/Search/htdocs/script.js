...
$(document).ready(function() {
	...
	function openSearchDialog(files) {
		...
		$.get(window.location.pathname, { action: "getSearchForm", files: files }, function(response) {
			var dialog = $(response);
			initSearchDialog(dialog);
			dialog.dialog({ width: "auto", height: "auto", maxWidth: $(window).width()-100, closeText: $("#close").html(), dialogClass: "search",
	...
	function initResult(result) {
		...
		$('img', result).off("error.search").on("error.search", function() {
			var self = $(this);
			self.removeClass("thumbnail").attr("src","data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=");
		});
		result.MyTooltip();
	}
