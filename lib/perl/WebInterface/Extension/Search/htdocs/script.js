...
$(document).ready(function() {
	...
	$(".action.search").click(function(event) {
		...
		$.get(window.location.pathname, { ajax: "getSearchForm", files: ToolBox.getSelectedFiles(this) }, function(response) {
			...
			dialog.dialog({ width: "auto", height: "auto", maxWidth: $(window).width()-100, 
	...
	function startResultPoll(dialog) {
		dialog.data("resultpoll", window.setTimeout(function() {
			$.get(window.location.pathname, { ajax: "getSearchResult", searchid : $('input.search.id', dialog).val()}, 
					function(response) { 
	...
	function initResult(result) {
		...
		$('a.search.result.entry',result).hover(function() {
			...
			self.data("timeout",window.setTimeout(function() {
				$(".search.result.fileuri",self.parent()).hide(500);
			},5000));
