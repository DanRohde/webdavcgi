...
$(document).ready(function() {
	$("body").on("fileActionEvent", function(event,data) {
		...
		if (data.obj.hasClass('viewerjs')) {
			...
			var xhr = $.get(window.location.pathname, { action:"viewerjs", file:data.file}, function(response) {
				block.remove();
				var dialog = $(response);
				$(".close.button",dialog).button().click(function() { dialog.remove()});
				$(window)
					.off("popstate.viewerjs unload.viewerjs beforeunload.viewerjs")
					.on("popstate.viewerjs unload.viewerjs beforeunload.viewerjs", function(){ if (dialog) dialog.remove(); })			
