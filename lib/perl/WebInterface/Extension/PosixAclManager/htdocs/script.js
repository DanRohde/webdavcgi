...
PosixAclManager.prototype = {
	...
	show: function() {
		...
		$.post($("#fileList").data("uri"), {ajax: 'getPosixAclManager', files: self.files}, function(response) {
			...
			self.dialog.dialog({modal: true, width: "auto", height: "auto", closeText: $("#close").html(), resizable: true, close: function() { self.dialog.dialog("destroy"); $(".action.refresh:first").trigger("click");}});	
		});
