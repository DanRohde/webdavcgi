...
$(document).ready(function() {
	$("body").on('fileActionEvent', function(event, data) {
		...
		if (!data.obj.hasClass('disablebinarydownload')) {
			window.location.href = window.location.pathname + '?' + $.param({action:'dwnload',file:data.file});
		} else {
			var sep = window.location.pathname.lastIndexOf('/') + 1 == window.location.pathname.length ? '' : '/';
			window.location.href = window.location.pathname + sep + data.file;
		}
	});
});