...
$(document).ready(function() {
	...
	$(".action.zipcompress").click(function(event) {
		...
		var xhr = $.post(window.location.pathname, {action: 'zipcompress', files: ToolBox.getSelectedFiles(this)}, function (response) {
			block.remove();
	...
	$(".action.zipuncompress").click(function(event) {
		...
		ToolBox.confirmDialog($("#zipuncompressconfirm").html(), { confirm: function() {
				var block = ToolBox.blockPage();
				var xhr = $.post(window.location.pathname, { action: 'zipuncompress', files: ToolBox.getSelectedFiles(self)}, function(response) {
					block.remove();
					ToolBox.handleJSONResponse(response);
					ToolBox.updateFileList();
				});
