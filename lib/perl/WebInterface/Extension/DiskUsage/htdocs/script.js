...
$(document).ready(function() {
	...
	function du_initTreemap(dialog) {
		$(".treemappanel",dialog).each(function(i,v) {
			...
			var cache = new Array();
			$(v).treemap({ "animationEnabled": true, "animationDurationMs": 2000, 
							"colorResolution": 2048, "dimensions": [800,600], "nodeData": $(v).data("mapdata")
			})
				...
				.on("treemapmousemove", function(event,data) {
					...
					if (data.nodes[0].title) {
						...
						if (event.pageX + elWidth > treemapOffset.left + treemapWidth) left-=elWidth;
						if (event.pageY + elHeight > treemapOffset.top + treemapHeight) top-=elHeight;
