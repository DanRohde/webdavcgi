...
$(document).ready(function() {
	...
	function du_calculateColorStops(steps) {
		// calculate gradient from purple to red over blue, green, yellow
		...
		var g = min;
		var b = max;
		var sw = (max-min) * 5 / steps;
		var phase = 1;
		for (var i=0; i<=steps; i++) {
			var rb = Math.round(b);
			var rg = Math.round(g);
			var rr = Math.round(r)
			var rgb = rb | rg << 8 | rr << 16;
			...
			if (phase == 5 && g == min) phase =1;
 			if ((phase == 4 && r == max) || (phase == 3 && b == min) || (phase == 2 && g == max)|| (phase == 1 && r == min)) phase++;
		}
		return colorStops;
	}
