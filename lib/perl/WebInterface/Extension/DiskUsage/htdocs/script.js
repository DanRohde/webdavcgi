...
$(document).ready(function() {
	...
	function du_dialog(files) {
		...
		var xhr = $.post($("#fileList").attr("data-uri"),{action: "diskusage", file: files}, function (response) {
			...
			dialog.dialog({modal: true, width: "auto", height: "auto", maxWidth: $(window).width()-100, resizable: true, 
					open: function() { du_initTreemap(dialog); du_initStatistics(dialog); du_initAccordion(dialog); }, 
	...
	function du_initStatistics(dialog) {
		$(".diskusage.statistics.piechart",dialog).each(function (i,v) {
			var self = $(v);
			$.jqplot(self.attr("id"), [ self.data("json").data ],{
					seriesDefaults: {
					...
					legend: { show:true, rendererOptions: { numberRows: 2}, location: 's' }
			});
			self.bind('jqplotDataHighlight', function(ev, seriesIndex, pointIndex, data) {
	            var title = data[0] + ": " + ( self.hasClass("size") ?   ToolBox.renderByteSize(data[1]) : data[1]);
	            $(this).attr('title', title);
	            $("#"+self.attr("id")+"-info").html(title);
	        }).bind('jqplotDataUnhighlight', function(ev, seriesIndex, pointIndex, data) {
	            $(this).attr('title',"");
	            $("#"+self.attr("id")+"-info").html("");
	        });
		}); 
		var acc = $(".diskusage.histograms.accordion",dialog);
		acc.on("MyActivate", function(e,ui) {
			console.log(ui);
			for (var i in acc.data("plots")) acc.data("plots")[i].redraw();
		});
		acc.data("plots", new Array());
		$(".diskusage.statistics.barchart",dialog).each(function(i,v) {
			var self = $(v);
			var json = self.data("json");
			acc.data("plots").push($.jqplot(self.attr("id"), [ json.data ], {
				title: { text: self.attr("title") },
				height: 700,
				width: 300,
                seriesDefaults : { renderer: $.jqplot.BarRenderer, rendererOptions: {fillToZero: true }, pointLabels : { show: true, location: 's' } },
                axes: {
                                xaxis : { label: self.data("xlabel"), renderer: $.jqplot.CategoryAxisRenderer },
                                yaxis : { label: self.data("ylabel"), labelRenderer: $.jqplot.CanvasAxisLabelRenderer, min: 0 }
                }
			}));
		});
