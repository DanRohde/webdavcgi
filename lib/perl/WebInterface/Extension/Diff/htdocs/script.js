...
$(document).ready(function() {
	$("body").on("fileActionEvent", function(event,data) {
			...
			if (response.error) {
				...
				function initButtons() {
					...
					$('.diff.swapfiles', dialog).button().click(function() {
						...
						$.post(window.location.pathname, {action:'diff', files: new Array($(this).data("file2"), $(this).data("file1"))}, handleResponse);
					});
				}
		...
		$.post(window.location.pathname, {action:'diff', files: data.selected}, handleResponse);
