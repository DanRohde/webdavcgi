...
sub _render_extension_element {
    ...
    if ( ref($a) eq 'HASH' ) {
        if ( ${$a}{subpopupmenu} ) {
            return ${$self}{cgi}->li(
                ...
                ( ${$a}{title} || q{} )
                    . ${$self}{cgi}->ul(
                    { -class => 'subpopupmenu extension' },
                    $self->_render_extension_element(
                        $fn, $ru, ${$a}{subpopupmenu}
                    )
                    )
            );
        }
        ...
        $content = join q{},
            map { $self->_render_extension_element( $fn, $ru, $_ ) } @{$a};
    }
    else {
        $content .= $a;
    }
...
sub _render_extension {
    ...
    return join q{},
        map { $self->_render_extension_element( $fn, $ru, $_ ) }
        @{ ${$self}{config}{extensions}->handle( $hook, { path => $fn } )
            // [] };
}
...
sub _render_file_list_table {
    ...
    $jsondata{quicknav}
        = $self->minify_html( $self->_render_quicknav_path( $fn, $ru ) );
    my $json = JSON->new();
...
sub _render_file_list_entry {
    ...
    my %stdvars = (
        ...
        ? ${$self}{cgi}->escapeHTML($file)
            . ' &rarr; '
            . ${$self}{cgi}
            ->escapeHTML( ${$self}{backend}->getLinkSrc($full) )
        : q{},
    );
