...
sub renderTemplate {
	...
	my %stdvars = ( uri => $ru, 
			...
			stat_loadedperlmodules => $CACHE{renderTemplate}{stat_loadedperlmodules} //= keys(%INC) +1,
			stat_perlmodulelist => $CACHE{renderTemplate}{stat_perlmodulelist} //= join(', ', sort keys %INC),
...
sub renderExtensionElement {
	...
	if (ref($a) eq 'HASH') {
		if ($$a{subpopupmenu}) {
			return $$self{cgi}->li({-class=>'subpopupmenu extension '. ($$a{classes} || '')},($$a{title} || '').$$self{cgi}->ul({-class=>'subpopupmenu extension'},$self->renderExtensionElement($fn,$ru,$$a{subpopupmenu}))) ;
		}
		my %params = (-class=>'');
...
sub renderFileListEntry {
	...
	my %stdvars = ( 
				...
				'islink' =>$il ? 'yes' : 'no',
				'isempty'=> $id ? 'unknown' : (defined $size) && $size == 0 ? 'yes' : 'no',
