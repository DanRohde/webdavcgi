...
sub new {
    ...
    $$self{config}{extensions}
        = WebInterface::Extension::Manager->new( $$self{config}, $$self{db} );
...
sub optimizeCssAndJs {
    ...
    my $content = $self->optimizer_ExtractContentFromTagsAndAttributes( $tags,
        'css' );
    $self->optimizer_writeContent2Zip( $csstargetfile, \$content )
        if $content;
    ...
    $content
        = $self->optimizer_ExtractContentFromTagsAndAttributes( $tags, 'js' );
...
sub optimizer_Collect {
    ...
    if ($filename) {
        ...
        if ( $type eq 'css' ) {
            ...
            $fc =~
                s/url\((.*?)\)/$self->optimizer_encodeImage($basepath, $1)/iegxs;
        }
        $$contentref .= $fc;
...
sub optimizer_ExtractContentFromTagsAndAttributes {
    ...
    if ( $type eq 'css' ) {
        $data =~
            s@<style[^>]*>(.*?)</style>@$self->optimizer_Collect(\$content, undef, $1, $type)@xiegs;
        $data =~
            s@<link[ ].*?href="(.*?)"@$self->optimizer_Collect(\$content, $1, undef, $type)@xiegs;
    }
    else {
        $data =~
            s@<script[ ].*?src="([^>"]+)".*?>(.*?)</script>@$self->optimizer_Collect(\$content, $1, $2, $type)@xiegs;
    }
