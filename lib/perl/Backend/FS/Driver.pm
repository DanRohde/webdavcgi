...
sub exists {
    return
        exists $CACHE{ $_[0] }{ $_[1] }{exists}
        && defined $CACHE{ $_[0] }{ $_[1] }{exists}
        ? $CACHE{ $_[0] }{ $_[1] }{exists}
        : ( $CACHE{ $_[0] }{ $_[1] }{exists}
            = -e $_[0]->resolveVirt( $_[1] ) );
}

sub isDir {
    return
        exists $CACHE{ $_[0] }{ $_[1] }{isDir}
        ? $CACHE{ $_[0] }{ $_[1] }{isDir}
        : ( $CACHE{ $_[0] }{ $_[1] }{isDir}
            = -d $_[0]->resolveVirt( $_[1] ) );
}

sub isFile {
    return
        exists $CACHE{ $_[0] }{ $_[1] }{isFile}
        ? $CACHE{ $_[0] }{ $_[1] }{isFile}
        : ( $CACHE{ $_[0] }{ $_[1] }{isFile}
            = -f $_[0]->resolveVirt( $_[1] ) );
}
...
sub isReadable {
    return
        exists $CACHE{ $_[0] }{ $_[1] }{isReadable}
        && defined $CACHE{ $_[0] }{ $_[1] }{isReadable}
        ? $CACHE{ $_[0] }{ $_[1] }{isReadable}
        : ( $CACHE{ $_[0] }{ $_[1] }{isReadable}
...
sub isWriteable {
    return
        exists $CACHE{ $_[0] }{ $_[1] }{isWriteable}
        && defined $CACHE{ $_[0] }{ $_[1] }{isWriteable}
        ? $CACHE{ $_[0] }{ $_[1] }{isWriteable}
        : ( $CACHE{ $_[0] }{ $_[1] }{isWriteable}
    ...
    if ( !main::isAllowed( $f, 1 ) ) {
        ...
        if ( opendir( my $dirh, $self->resolveVirt($f) ) ) {
            foreach my $sf ( grep { !/^[.]{1,2}$/xms } readdir($dirh) ) {
                my $full = $f . $sf;
                $full .= '/' if $self->isDir($full) && $full !~ /\/$/xms;
...
sub saveStream {
    ...
    if ( ( $ret = open( my $f, '>', $self->resolveVirt($destination) ) ) ) {
        if ( $main::ENABLE_FLOCK && !flock( $f, LOCK_EX | LOCK_NB ) ) {
            ...
            my ($consumed) = 0;
            while (
                read( $filehandle, my $buffer, $main::BUFSIZE || 1048576 )
                > 0 )
            {
                last
                    if $block_hard > 0
            ...
            close($f);
            $ret = 0
                if $block_hard > 0 && $consumed + $block_curr >= $block_hard;
        }
    }
    else {
        $ret = 0;
...
sub createSymLink {
    delete $CACHE{ $_[1] }{ $_[2] };
    return CORE::symlink( $_[0]->resolveVirt( $_[1] ),
        $_[0]->resolveVirt( $_[2] ) );
}
...
sub getFileContent {
    ...
    if (   $self->exists($fn)
        && !$self->isDir($fn)
        && open( my $fh, "<", $self->resolveVirt($fn) ) )
    {
        binmode $fh;
...
sub rename {
    ...
    return main::isAllowed( $_[0]->resolveVirt( $_[1] ), 1 )
        ? CORE::rename( $_[0]->resolveVirt( $_[1] ),
        $_[0]->resolveVirt( $_[2] ) )
        : 0;
}
...
sub copy {
    ...
    if (   open( my $srcfh, '<', $self->resolveVirt( $src, 1 ) )
        && open( my $dstfh, '>', $self->resolveVirt( $dst, 1 ) ) )
    {
...
sub isVirtualLink {
    ...
    return
        ...
        { $self->dirname($fn) . '/' }
        && exists $main::BACKEND_CONFIG{$main::BACKEND}{fsvlink}
        { $self->dirname($fn) . '/' }{ $self->basename($fn) };
}
