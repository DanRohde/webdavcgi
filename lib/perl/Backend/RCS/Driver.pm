...
sub stat {
	my $self = shift @_;
	return ( 0,0, 0555, 2, $<, $(, 0,0, time(),time(),time(), 4096,0 )  if ($self->_isVirtualDir($_[0]));
	if ($self->_isVirtualFile($_[0])) {
		my $lf = $self->_saveToLocal($_[0]);
		my @stat = CORE::stat($lf);
		unlink $lf;
		return @stat;
	}
	return $$self{BACKEND}->stat(@_);
}
sub lstat {
	my $self = shift @_;
...
sub saveData {
	...
	return 0 if ($self->_isVirtualDir($destination));
	my $ret = 0;
...
sub printFile {
	...
	if ($self->_isVirtualFile($fn)) {
		...
		if ($fn=~/log.txt$/) {
			...
			$fn=~/\/(\d+\.\d+)\/[^\/]+$/;
			my $rev = $1;
			if ($rcs->co("-r$rev") && open(my $lfh, "<$file")) {
				binmode($fh);
				binmode($lfh);
