...
function startAutoRefresh(timeout) {
	document.getElementsByName('autorefreshtime')[0].options[0].selected=true;
	if (timeout == '-') {
		return false;
	} else if (timeout == 'now') {
		pleaseWait();
		...
		return false;
	} else if (timeout == 'toggle') {
		toggleAutoRefresh();
		return false;
	} else if (timeout == 'clear' || timeout<=0 || !document.getElementById('autorefreshtimer')) {
		stopAutoRefresh();
		return false;
	}
	...
	document.getElementById('autorefresh_toggle').disabled=false;
	document.getElementById('autorefresh_clear').disabled=false;
...
function stopAutoRefresh(dontforget) {
	...
	document.getElementById('autorefresh_toggle').disabled=true;
	document.getElementById('autorefresh_clear').disabled=true;
