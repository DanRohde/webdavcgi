...
function autorefreshcheck() {
	if (getCookie("autorefresh") != "") setAutoRefresh(getCookie("autorefresh"));
}
function check() {
	selcheck();
	clpcheck();
...
function setAutoRefresh(seconds) {
	...
	if (seconds>0 && !window.location.search.match(/edit/)) {
		autorefreshinterval = window.setInterval("window.location.reload()",1000 * seconds);
	}		
	setCookie('autorefresh',seconds,1);
}
function applyFilters() {
	var els = document.getElementsByName('filter.types');
	var typefilter = "";
	var path = false;
	for (var i=0; i<els.length; i++) {
		if (els[i].checked) typefilter+=els[i].value;
	}
	setCookie('filter.types', typefilter, 1, path ? window.location.pathname : 0);
	...
	setCookie('filter.size', sizeop+sizeval+sizeunit, 1, path ? window.location.pathname : 0);

	window.location.reload();
	return 0;
}
function resetFilters() {
	var cookies = new Array('filter.size','filter.time','filter.types');
