...
sub HTTP_GET {
        ...
        && !$backend->isReadable($PATH_TRANSLATED) )
    {
        ...
        if (   $ENABLE_COMPRESSION
            ...
            && $stat[7] > 1023
            && $mime =~ /^(text\/(css|html)|application\/(x-)?javascript)$/xmsi )
        {
            my %header = (
                -status => '200 OK',
                -type   => $mime,
            ...
            if ( defined $start ) {
                ...
                $header{-Content_Range}
                    = sprintf( 'bytes %s-%s/%s', $start, $end, $stat[7] );
            ...
            if (open(
                    my $F, '<',
                    $backend->getLocalFilename($PATH_TRANSLATED)
                )
                )
            {
...
sub HTTP_PUT {
    ...
    if ( defined $cgi->http('Content-Range') ) {
        ...
        && !$backend->isWriteable( $backend->getParent( ($PATH_TRANSLATED) ) )
        )
    {
...
sub HTTP_MOVE {
    ...
    elsif (!isAllowed( $PATH_TRANSLATED, $backend->isDir($PATH_TRANSLATED) )
        || !isAllowed( $destination, $backend->isDir($destination) ) )
    {
        $status = '423 Locked';
    }
    else {
        ...
        || ( defined $ENV{QUERY_STRING} && $ENV{QUERY_STRING} ne "" ) )
    {
        ...
        if ($ENABLE_TRASH) {
            ...
            for my $diag (@err) {
                ...
                push @resps,
                    { href => $file, status => "403 Forbidden - $message" };
            }
...
sub HTTP_LOCK {
    ...
    my $ru = $REQUEST_URI;
    my $depth
        = defined $cgi->http('Depth') ? $cgi->http('Depth') : 'infinity';
...
sub printCompressedHeaderAndContent {
    ...
    return printHeaderAndContent( $status, $type, $content, $header,
        $cookies );
}
